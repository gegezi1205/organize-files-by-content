...
- SkillHub 中文显示名称改为 `文件｜按内容整理与自动归档`，公开概述改为短文案，前两句直接说明 `00_待归档`、自动改名归档和歧义文件原位保留。
- 增加三张中文说明图，分别说明投放箱流程、文件去向的判断依据和正式材料保护边界。
- GitHub README 保留中英双语，英文介绍按实际流程重新撰写，不沿用中文句式直译。
- 完整执行规则移到 `references/organize-workflow.md`，`SKILL.md` 保留清楚的执行入口。规则含义未删减或改变。

### 边界

- 未修改 `organizer.py`、`watch_inbox.py`、`install_autostart.py`、`evals/`、配置格式、分类门槛或安全检查。
- `self_test.py` 只同步预期版本和完整规则的新路径，原有测试场景与断言内容不变。
- 本版只调整展示文案、说明图、文档结构及对应版本记录。
